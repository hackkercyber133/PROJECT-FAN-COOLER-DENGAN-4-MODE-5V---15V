...
## 💡 Lighting Matrix — 20 mode LED + mode Custom (v2)
Strip LED yang dipakai adalah **RGB addressable 3 kabel (WS2812/NeoPixel)** di `PIN_LED_DATA` (GPIO 4, `NUM_LEDS=30`).
- Tombol **daya** (power) nyala/mati pakai efek terakhir yang dipilih.
- Tombol **"MODE AKTIF"** membuka menu berisi **20 mode efek siap pakai** (statis, pelangi jalan, teater, bernapas, disko, pantul, api, komet, kerlip, gelombang, sapuan warna, strobo, polisi, meteor, gradasi, dll) + 1 tile **Custom**.
- **Custom** membuka roda warna bulat ala **ROG Aura** (geser jari di cincin pelangi untuk pilih warna, logo **VP 3D** animasi persis di tengah bulatan) plus pilihan gaya gerakan sendiri (diam/kejar/teater/bernapas/komet/gelombang/meteor/kerlip), arah (maju/mundur), kecepatan & kecerahan.
- **Kecepatan** & **Kecerahan** diatur lewat slider dan berlaku untuk mode apa pun yang sedang aktif.
- Semua pengaturan LED (mode, warna, kecepatan, kecerahan, arah) tersimpan di NVS ESP32 (`Preferences`) sehingga tetap sama setelah restart.

Perintah dikirim sebagai JSON lewat BLE (characteristic yang sama) atau lewat HTTP lokal `GET /api/led` dengan query `mode`, `speed` (0-100), `brightness` (0-100), `color` (hex `RRGGBB`), `direction` (`1`/`-1`), dan `customStyle` (khusus mode `custom`). Contoh: `/api/led?mode=comet&speed=70&brightness=90&color=00F0FF&direction=1`.
