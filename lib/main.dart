...
// Dialog custom dengan transisi scale+fade yang smooth (pengganti showDialog
// polos bawaan Flutter yang muncul instan/kaku), dipakai di semua dialog
// aplikasi supaya terasa hidup dan konsisten dengan tema cyber/gaming.
...
const List<Map<String, dynamic>> kLedModeCatalog = [
  ...
  {"id": "twinkle", "label": "Kelap-Kelip", "icon": Icons.star_border},
  {"id": "twinkle_rainbow", "label": "Kelap Pelangi", "icon": Icons.grain},
  {"id": "confetti", "label": "Konfeti", "icon": Icons.celebration},
  {"id": "cylon", "label": "Mata Cylon", "icon": Icons.remove_red_eye},
  {"id": "dual_scan", "label": "Pindai Ganda", "icon": Icons.compare_arrows},
  {"id": "rain", "label": "Hujan", "icon": Icons.water_drop},
  {"id": "fireworks", "label": "Kembang Api", "icon": Icons.auto_awesome},
  {"id": "plasma", "label": "Plasma", "icon": Icons.blur_on},
  {"id": "noise", "label": "Riak Cahaya", "icon": Icons.graphic_eq},
  {"id": "lightning", "label": "Petir", "icon": Icons.bolt},
  {"id": "bpm", "label": "Detak BPM", "icon": Icons.monitor_heart},
  {"id": "juggle", "label": "Bola Juggling", "icon": Icons.scatter_plot},
  {"id": "pride", "label": "Gelombang Warna", "icon": Icons.waves},
  {"id": "fade", "label": "Pudar Silang", "icon": Icons.opacity},
  {"id": "blink", "label": "Kedip", "icon": Icons.flash_on},
  {"id": "blink_rainbow", "label": "Kedip Pelangi", "icon": Icons.flash_auto},
  {"id": "dissolve", "label": "Larut", "icon": Icons.blur_circular},
  {"id": "dissolve_rainbow", "label": "Larut Pelangi", "icon": Icons.blur_linear},
  {"id": "running_dual", "label": "Lari Berlawanan", "icon": Icons.swap_horizontal_circle},
  {"id": "traffic_light", "label": "Lampu Lalin", "icon": Icons.traffic},
  {"id": "halloween", "label": "Halloween", "icon": Icons.nightlight_round},
  {"id": "christmas", "label": "Natal", "icon": Icons.park},
  {"id": "ocean", "label": "Ombak Laut", "icon": Icons.water},
  {"id": "aurora", "label": "Aurora", "icon": Icons.blur_on},
  {"id": "candle", "label": "Lilin Berkedip", "icon": Icons.whatshot},
  {"id": "colorloop", "label": "Putar Warna", "icon": Icons.loop},
  {"id": "saw", "label": "Gergaji", "icon": Icons.show_chart},
  {"id": "multi_comet", "label": "Komet Ganda", "icon": Icons.stars},
  {"id": "two_dots", "label": "Dua Titik", "icon": Icons.more_horiz},
  {"id": "lighthouse", "label": "Mercusuar", "icon": Icons.wb_incandescent},
];
...
class _SplashScreenState extends State<SplashScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _bolts = _buildLightningBolts(Random(42));
    ...
    return Scaffold(
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          // Denyut kilat di logo: menyala terang tiap kali ada sambaran petir
          // yang sedang di puncak intensitasnya, biar logo terasa "hidup".
          double boltGlow = 0;
          for (final b in _bolts) {
            final local = ((t - b.startT) / b.duration).clamp(0.0, 1.0);
            if (t >= b.startT && t <= b.startT + b.duration) {
              final flash = local < 0.25 ? (local / 0.25) : (1 - (local - 0.25) / 0.75);
              boltGlow = max(boltGlow, flash * b.intensity);
            }
          }
          ...
          return Stack(
            ...
            children: [
              ...
              // Sambaran petir yang menyambar di sekitar & menembus logo -
              // digambar sebelum logo supaya kilatnya terasa "keluar dari" logo.
              Center(
                child: IgnorePointer(
                  child: Opacity(
                    opacity: (0.55 + 0.45 * ringIntro).clamp(0.0, 1.0),
                    child: CustomPaint(
                      painter: _LightningPainter(bolts: _bolts, t: t, colorA: neon, colorB: neon2),
                      ...
                      child: Transform.scale(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: neon.withOpacity((0.5 + 0.25 * sin(loop * 2 * pi).abs() + boltGlow * 0.5).clamp(0.0, 1.0)),
                                blurRadius: 46 + boltGlow * 40,
                                spreadRadius: 4 + boltGlow * 10,
                              ),
                              BoxShadow(
                                color: neon2.withOpacity((0.25 + boltGlow * 0.35).clamp(0.0, 1.0)),
                                blurRadius: 60 + boltGlow * 30,
                                spreadRadius: 2 + boltGlow * 6,
                              ),
                              if (boltGlow > 0.05)
                                BoxShadow(
                                  color: Colors.white.withOpacity(boltGlow * 0.55),
                                  blurRadius: 20 + boltGlow * 30,
                                  spreadRadius: boltGlow * 4,
                                ),
                            ],
                          ),
...
// ===== PETIR (lightning) - dibangkitkan via midpoint-displacement supaya =====
// ===== bentuknya patah-patah alami seperti sambaran listrik asli =====
...
Path _jaggedPath(Offset a, Offset b, Random rnd, {int depth = 5, double displace = 46}) {
  final points = <Offset>[a, b];
  double amp = displace;
  for (int d = 0; d < depth; d++) {
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      final nx = len == 0 ? 0 : -dy / len;
      final ny = len == 0 ? 0 : dx / len;
      final offset = (rnd.nextDouble() * 2 - 1) * amp;
      next.add(Offset(mid.dx + nx * offset, mid.dy + ny * offset));
      next.add(p2);
    }
    points
      ..clear()
...
List<_LightningBolt> _buildLightningBolts(Random rnd) {
  const c = Offset(150, 150);
  final bolts = <_LightningBolt>[];

  // Sambaran utama: menembus logo dari atas ke bawah, muncul saat logo baru "hidup".
  final mainStart = Offset(c.dx + (rnd.nextDouble() * 30 - 15), 8);
  final mainEnd = Offset(c.dx + (rnd.nextDouble() * 30 - 15), 292);
  final mainPath = _jaggedPath(mainStart, mainEnd, rnd, depth: 5, displace: 40);
  final branches = <Path>[];
  for (int i = 0; i < 3; i++) {
    final t0 = 0.25 + i * 0.2 + rnd.nextDouble() * 0.1;
    final metric = mainPath.computeMetrics().first;
    final tan = metric.getTangentForOffset(metric.length * t0);
    if (tan != null) {
      final branchEnd = tan.position +
          Offset((rnd.nextDouble() * 2 - 1) * 60, 20 + rnd.nextDouble() * 30);
      branches.add(_jaggedPath(tan.position, branchEnd, rnd, depth: 3, displace: 20));
    }
  }
  bolts.add(_LightningBolt(
    main: mainPath, branches: branches, startT: 0.30, duration: 0.16, intensity: 1.0, magenta: false,
  ));

  // Sambaran-sambaran kecil susulan di sekitar cincin, biar terasa terus "hidup".
  final smallSchedule = [0.52, 0.66, 0.80];
  for (final st in smallSchedule) {
    final angle = rnd.nextDouble() * 2 * pi;
    final r1 = 95.0, r2 = 145.0;
    final p1 = Offset(c.dx + cos(angle) * r1, c.dy + sin(angle) * r1);
    final p2 = Offset(c.dx + cos(angle) * r2, c.dy + sin(angle) * r2);
    bolts.add(_LightningBolt(
      main: _jaggedPath(p1, p2, rnd, depth: 3, displace: 18),
      branches: const [],
      startT: st,
      duration: 0.10,
      intensity: 0.7,
...
class _LightningPainter extends CustomPainter {
  ...
  final Color colorA;
  final Color colorB;
  _LightningPainter({required this.bolts, required this.t, required this.colorA, required this.colorB});

  @override
  void paint(Canvas canvas, Size size) {
    for (final b in bolts) {
      if (t < b.startT || t > b.startT + b.duration) continue;
      final local = (t - b.startT) / b.duration;
      final flash = local < 0.3 ? (local / 0.3) : (1 - (local - 0.3) / 0.7);
      final op = flash.clamp(0.0, 1.0) * b.intensity;
      if (op <= 0.01) continue;

      final color = b.magenta ? colorB : colorA;
      ...
      canvas.drawPath(b.main, glowPaint);
      canvas.drawPath(b.main, corePaint);
      for (final branch in b.branches) {
        final branchGlow = Paint()
          ..color = color.withOpacity(0.4 * op)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 6
          ..strokeCap = StrokeCap.round
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 6);
        final branchCore = Paint()
          ..color = Colors.white.withOpacity(0.85 * op)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 1.4
          ..strokeCap = StrokeCap.round;
        canvas.drawPath(branch, branchGlow);
        canvas.drawPath(branch, branchCore);
      }
    }
  }

  @override
  bool shouldRepaint(covariant _LightningPainter oldDelegate) => oldDelegate.t != t;
}
...
class _ControllerPageState extends State<ControllerPage> with TickerProviderStateMixin {
                    ...
                    children: [
                      AnimatedBuilder(
                        ...
                        builder: (context, _) {
                          ...
                          final pulse = isOnline ? _pulseCtrl.value : 0.0;
                          final c = isOnline ? CyberColors.success : CyberColors.danger;
                          return Container(
                            width: 7 + pulse * 2,
                            height: 7 + pulse * 2,
                            decoration: BoxDecoration(
                              ...
                              color: c,
                              boxShadow: [
                                BoxShadow(color: c.withOpacity(0.5 + pulse * 0.4), blurRadius: 6 + pulse * 8),
                              ],
                            ),
                          );
                        },
  ...
  List<Color> _previewLedColors(double t) {
    ...
    switch (activeMode) {
      ...
      case "twinkle":
      case "twinkle_rainbow":
        final seed = (t * speed * 12).floor();
        return List.generate(dots, (i) {
          final on = ((seed * 7 + i * 13) % 10) < 2;
          if (!on) return off[i];
          return dim(activeMode == "twinkle_rainbow"
              ? _previewRainbow((i * 40.0 + seed * 17) % 360)
              : base);
        });

      case "confetti":
        final seed2 = (t * speed * 14).floor();
        return List.generate(dots, (i) {
          final on = ((seed2 * 5 + i * 9) % 10) < 3;
          if (!on) return off[i];
          return dim(_previewRainbow((i * 61 + seed2 * 53) % 360));
        });

      case "cylon":
        final cyclePosA = (t * speed * (dots - 1) * 2) % ((dots - 1) * 2);
        final headA = cyclePosA <= dots - 1 ? cyclePosA : 2 * (dots - 1) - cyclePosA;
        return List.generate(dots, (i) {
          final dist = (headA - i).abs();
          return dist <= 1 ? dim(base) : off[i];
        });

      case "dual_scan":
        final cyclePosB = (t * speed * (dots - 1) * 2) % ((dots - 1) * 2);
        final headB = cyclePosB <= dots - 1 ? cyclePosB : 2 * (dots - 1) - cyclePosB;
        return List.generate(dots, (i) {
          final on = i == headB.round() || i == (dots - 1 - headB).round();
          return on ? dim(base) : off[i];
        });

      case "rain":
        final drop = (t * speed * dots).floor() % dots;
        return List.generate(dots, (i) {
          final d = direction > 0 ? (drop - i + dots) % dots : (i - drop + dots) % dots;
          final intensity = (1.0 - d / 3.0).clamp(0.0, 1.0);
          return dim(base, intensity);
        });

      case "fireworks":
        final burst = (t * speed * 6).floor();
        final center = (burst * 7) % dots;
        return List.generate(dots, (i) {
          final dist = (i - center).abs();
          final intensity = (1.0 - dist / 2.0).clamp(0.0, 1.0);
          return dim(_previewRainbow((burst * 43) % 360), intensity);
        });

      case "plasma":
        return List.generate(dots, (i) {
          final v = sin(i * 0.6 + t * speed * 4) + sin(i * 0.3 - t * speed * 3);
          return dim(_previewRainbow(((v + 2) / 4) * 360));
        });

      case "noise":
        return List.generate(dots, (i) {
          final n = (sin(i * 0.9 + t * speed * 3) * sin(i * 0.4 - t * speed * 2) + 1) / 2;
          return dim(base, n);
        });

      case "lightning":
        final flash = sin(t * speed * 30) > 0.85;
        return List<Color>.filled(dots, flash ? Colors.white : off.first);

      case "bpm":
        final pulse2 = 0.2 + 0.8 * ((sin(t * speed * 3 * pi) + 1) / 2);
        return List.generate(
          dots,
          (i) => dim(_previewRainbow((i * 30 + t * speed * 200) % 360), pulse2),
        );

      case "juggle":
        final p1 = ((sin(t * speed * 2 + 0) + 1) / 2 * (dots - 1)).round();
        final p2 = ((sin(t * speed * 2.7 + 2) + 1) / 2 * (dots - 1)).round();
        final p3 = ((sin(t * speed * 3.4 + 4) + 1) / 2 * (dots - 1)).round();
        return List.generate(dots, (i) {
          final on = i == p1 || i == p2 || i == p3;
          return on ? dim(_previewRainbow(i * 40.0)) : off[i];
        });

      case "pride":
        return List.generate(
          dots,
          (i) => dim(_previewRainbow((i * 22 + t * speed * 240) % 360)),
        );

      case "fade":
        final fb = (sin(t * speed * 2 * pi) + 1) / 2;
        return List<Color>.filled(
          dots,
          dim(Color.lerp(base, _previewRainbow(t * speed * 300), fb) ?? base),
        );

      case "blink":
      case "blink_rainbow":
        final blinkOn = (t * speed * 3).floor().isEven;
        if (!blinkOn) return off;
        return List<Color>.filled(
          dots,
          dim(activeMode == "blink_rainbow" ? _previewRainbow(t * speed * 360) : base),
        );

      case "dissolve":
      case "dissolve_rainbow":
        final frac2 = (t * speed) % 1.0;
        final filled = (frac2 * dots).floor();
        return List.generate(dots, (i) {
          if (i > filled) return off[i];
          return dim(activeMode == "dissolve_rainbow" ? _previewRainbow(i * 36.0) : base);
        });

      case "running_dual":
        final posA = (t * speed * dots).floor() % dots;
        final posB = dots - 1 - posA;
        return List.generate(dots, (i) {
          if (i == posA) return dim(base);
          if (i == posB) return dim(_previewRainbow(posA * 30.0));
          return off[i];
        });

      case "traffic_light":
        final phase2 = (t * speed * 1.5).floor() % 3;
        final tColor = phase2 == 0
            ? CyberColors.rog
            : (phase2 == 1 ? CyberColors.amber : CyberColors.success);
        return List<Color>.filled(dots, dim(tColor));

      case "halloween":
        return List.generate(dots, (i) {
          final purpleSlot = ((i + (t * speed * dots).floor()) % 4) < 2;
          return dim(purpleSlot ? const Color(0xFF8C00C8) : const Color(0xFFFF5A00));
        });

      case "christmas":
        return List.generate(dots, (i) {
          final redSlot = ((i + (t * speed * dots).floor()) % 2) == 0;
          return dim(redSlot ? Colors.red : Colors.green);
        });

      case "ocean":
        return List.generate(dots, (i) {
          final b = (sin(i * 0.6 + t * speed * 4) + 1) / 2;
          return dim(Color.lerp(const Color(0xFF003050), const Color(0xFF00C8FF), b) ?? base);
        });

      case "aurora":
        return List.generate(dots, (i) {
          final p1 = sin(i * 0.5 + t * speed * 3);
          final p2 = sin(i * 0.3 - t * speed * 2);
          return dim(_previewRainbow(140 + p1 * 40 + p2 * 20),
              ((p1 + p2 + 2) / 4).clamp(0.2, 1.0));
        });

      case "candle":
        return List.generate(dots, (i) {
          final flicker = 0.6 + 0.4 * sin(t * speed * 20 + i * 3.1);
          return dim(const Color(0xFFFFA020), flicker);
        });

      case "colorloop":
        return List<Color>.filled(dots, dim(_previewRainbow(t * speed * 200)));

      case "saw":
        final sawFrac = (t * speed) % 1.0;
        return List<Color>.filled(dots, dim(base, sawFrac));

      case "multi_comet":
        return List.generate(dots, (i) {
          for (final k in [0, 1, 2]) {
            final chead = (wrappedHead + k * (dots ~/ 3)) % dots;
            final dist = (chead - i).abs();
            if (dist < 2) return dim(_previewRainbow(k * 120.0), 1 - dist / 2);
          }
          return off[i];
        });

      case "two_dots":
        final ta = ((sin(t * speed * 2) + 1) / 2 * (dots - 1)).round();
        final tb = dots - 1 - ta;
        return List.generate(dots, (i) {
          if (i == ta) return dim(base);
          if (i == tb) return dim(_previewRainbow(180));
          return off[i];
        });

      case "lighthouse":
        return List.generate(dots, (i) {
          final dist = ((wrappedHead - i) % dots + dots) % dots;
          final intensity = dist < 2 ? (1 - dist / 2) : 0.08;
          return dim(base, intensity);
        });
